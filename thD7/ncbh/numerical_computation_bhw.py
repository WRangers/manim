...
class Epilogue(Scene):
    def construct(self):
        ...
        ak_text1=TextMobject(r"""
            \parbox[c][][c]{20em}{
                Special gratitude to Prof.Liang Wang and all the TAs for their instruction and insightful explanation!}
            """).shift(DOWN)
        ...
        pic_manim=ImageMobject('manim_graph').scale(1.5).move_to(np.array([-1.8,-2,0]))
        ...
        bgm_text=TextMobject('Reflections - Vincent Rubinetti').shift(DOWN*0.5)
        ...
        # References
        ref_title=TextMobject(r'\huge References').to_edge(UP,buff=1)
        # ref_text=TextMobject()

        self.play(
            *[FadeOut(i) for i in bgm],
            FadeIn(ref_title),
            # Write(ref_text)
        )
        self.wait(4)
